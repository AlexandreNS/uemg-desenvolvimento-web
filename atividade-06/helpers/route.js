...
function routerModelBuilder(schema, limitDefault = 20) {
  const router = express.Router()

  router.get('/', validateGetParams(schema), async (req, res) => {
  ...
  router.get('/:id', validateGetParams(schema), async (req, res) => {
  ...
  router.post('/', async (req, res) => {
    ...
    if (error) res.send({ error: error.details.map(({ message }) => message) })
